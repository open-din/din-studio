...
const DEFAULT_BRIDGE_PORT = Number(import.meta.env.VITE_DIN_STUDIO_MCP_BRIDGE_PORT ?? (import.meta.env.DEV ? 17374 : 17373));
const DEFAULT_BRIDGE_HOST = import.meta.env.VITE_DIN_STUDIO_MCP_HOST?.trim().toLowerCase() || 'localhost';
const DISCOVERY_PATH = '/bridge-info';
const SESSION_STORAGE_KEY = 'din-studio-agent-session-id';
let hasWarnedAboutTlsTrust = false;

function getDiscoveryProtocols(): Array<'http' | 'https'> {
    const configured = import.meta.env.VITE_DIN_STUDIO_MCP_PROTOCOL?.trim().toLowerCase();
...
async function discoverBridge(): Promise<BridgeDiscoveryInfo | null> {
    ...
    if (!hasWarnedAboutTlsTrust && lastError) {
        ...
        console.warn(
            import.meta.env.DEV
                ? 'DIN Studio MCP discovery failed. Start `npm run editor:dev:mcp` for local HTTP mode, or trust the local CA if you are using the secure standalone MCP server.'
                : 'DIN Studio MCP discovery failed. If this is the first secure run, trust the local MCP CA certificate generated by the MCP server, then reload the editor.',
            lastError,
        );
    }
