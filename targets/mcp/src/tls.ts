...
const CA_COMMON_NAME = 'DIN Studio MCP Local CA';
...
async function runOpenSsl(args: string[], cwd: string) {
    try {
        ...
        throw new Error(`OpenSSL failed while preparing DIN Studio MCP TLS certificates: ${message}`);
    }
}
...
export async function ensureLocalTlsBundle(options: {
    ...
    if (!caFresh) {
        ...
        options.logger.info('Generated DIN Studio MCP local certificate authority', {
    ...
    if (generatedNewAuthority || !serverFresh) {
        ...
        options.logger.info('Generated DIN Studio MCP HTTPS server certificate', {
            certDirectory: options.certDirectory,
    ...
    if (generatedNewAuthority) {
        options.logger.warn(
            'Trust the DIN Studio MCP local CA certificate in your browser or OS before using HTTPS/WSS from DIN Studio.',
            {
                caCertPath: paths.caCertPath,
                fingerprint256,
            },
