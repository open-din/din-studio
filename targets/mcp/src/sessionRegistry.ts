...
import type { BridgeEnvelope, BridgeHelloAck } from '../../../bridge/protocol';
import type {
    EditorSessionHello,
    EditorSessionState,
...
export class SessionRegistry {
    ...
    private removeSession(sessionId: string, reason: string) {
        ...
        this.options.logger.info('Removing DIN Studio session', {
            sessionId,
            reason,
        });
    ...
    resolveSession(sessionId?: string | null): SessionRecord {
        ...
        if (normalized) {
            ...
            if (!record) {
                throw new Error(`DIN Studio session "${normalized}" was not found.`);
            }
            return record;
        }
        ...
        if (sessions.length === 0) {
            throw new Error('No DIN Studio session is connected.');
        }
        if (sessions.length > 1) {
            throw new Error('Multiple DIN Studio sessions are connected. Provide "sessionId" explicitly.');
        }
    ...
    registerHello(socket: WebSocket, hello: EditorSessionHello): BridgeHelloAck {
        ...
        this.options.logger.info('Registered DIN Studio session', {
            sessionId,
            appVersion: hello.appVersion,
        ...
        if (envelope.requestId && this.pendingRequests.has(envelope.requestId)) {
            ...
            if (envelope.ok === false || envelope.type === 'session.error') {
                pending.reject(new Error(
                    ...
                    ?? `DIN Studio session "${sessionId}" returned an error for ${pending.type}.`,
                ));
                return;
            }
    ...
    async request<TPayload, TResponse>(
        ...
        if (record.socket.readyState !== WebSocket.OPEN) {
            throw new Error(`DIN Studio session "${sessionId}" is disconnected.`);
        }
        ...
        return new Promise<TResponse>((resolve, reject) => {
            const timeout = setTimeout(() => {
                this.pendingRequests.delete(requestId);
                reject(new Error(`Timed out waiting for "${type}" from DIN Studio session "${sessionId}".`));
            ...
            try {
                ...
                this.pendingRequests.delete(requestId);
                reject(new Error(errorMessage(error, `Failed to send "${type}" to DIN Studio session "${sessionId}".`)));
            }
        });
    }
}
