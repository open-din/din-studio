...
const PROJECTS_DB_NAME = 'din-studio-projects';
...
const LEGACY_AUDIO_DB_NAME = 'din-studio-audio-library';
const LEGACY_AUDIO_STORE = 'audioAssets';
const LEGACY_AUDIO_CACHE = 'din-studio-audio';
...
function getElectronBridge(): ElectronProjectBridgeApi | null {
    ...
    return isElectronBridge(window.dinStudioApp) ? window.dinStudioApp : null;
}
...
function getLegacyAudioCacheKey(assetId: string): string {
    const origin = typeof location !== 'undefined' ? location.origin : 'https://din-studio.local';
