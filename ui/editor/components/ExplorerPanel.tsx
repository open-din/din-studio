...
export const ExplorerPanel: FC<ExplorerPanelProps> = ({ onCreateGraph, onLoadGraph, onLoadTemplate, onDeleteGraph }) => {
    ...
    return (
        <div className="flex h-full flex-col bg-transparent">
                    ...
                    <div className="space-y-1">
                        ...
                        {graphs.map(graph => (
                            <div
                                ...
                                onContextMenu={(e) => {
                                    e.preventDefault();
                                    setContextMenu({ graphId: graph.id, x: e.clientX, y: e.clientY });
                                }}
            ...
            {contextMenu && (
                <div
                    className="fixed z-50 min-w-[140px] rounded-lg border border-[var(--panel-border)] bg-[var(--panel-bg)] py-1 shadow-lg"
                    style={{ left: contextMenu.x, top: contextMenu.y }}
                    onClick={(e) => e.stopPropagation()}
                >
                    <button
                        className="w-full px-3 py-1.5 text-left text-xs text-[var(--danger)] hover:bg-[var(--danger-soft)]"
                        onClick={() => {
                            onDeleteGraph?.(contextMenu.graphId);
                            setContextMenu(null);
                        }}
                    >
                        Delete Graph
