...
const PREFERRED_CATEGORY_ORDER: PaletteCategory[] = ['Sources', 'MIDI', 'Effects', 'Routing', 'Math'];
...
function sortCategoriesPresent(defs: StudioNodeDefinition[]): string[] {
    ...
    return present.sort((a, b) => {
        const ia = PREFERRED_CATEGORY_ORDER.indexOf(a as PaletteCategory);
        const ib = PREFERRED_CATEGORY_ORDER.indexOf(b as PaletteCategory);
        if (ia === -1 && ib === -1) {
                ...
                {categories.map((category) => {
                    ...
                    const categoryMeta =
                        category in CATEGORY_META
                            ? CATEGORY_META[category as PaletteCategory]
                            : { color: 'text-zinc-400', bg: 'bg-zinc-500/10' };
                    const CategoryIcon =
                        category in CATEGORY_ICONS ? CATEGORY_ICONS[category as PaletteCategory] : AudioLines;
                    ...
                    return (
                        <div key={category} className="space-y-3">
                            ...
                            {subKeys.map((sub) => {
                                ...
                                return (
                                    <div key={`${category}-${sub}`} className="space-y-2 pl-1">
                                        {sub !== 'General' && (
                                                        ...
                                                        <div className="relative flex items-center gap-3 rounded-lg border border-[var(--panel-border)] bg-[var(--panel-bg)] p-2.5 transition-all hover:bg-[var(--panel-muted)]/60">
                                                            ...
                                                            >
                                                                <EditorNodeIcon
                                                                    ...
                                                                    className={`${categoryMeta.color} h-4 w-4 transition-transform group-hover:scale-110`}
                                                                />
                                                            </div>
